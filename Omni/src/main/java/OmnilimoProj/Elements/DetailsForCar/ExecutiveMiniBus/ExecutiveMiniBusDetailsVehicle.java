...
public class ExecutiveMiniBusDetailsVehicle extends Boss{
	...
	public ExecutiveMiniBusDetailsVehicle() {
...
//	}
	...
	public void checkBaseCharges() {
		...
		assertEquals(CommonFunction.getLocator("$150.00").getText(), AdditionalFees.BaseChargesExecutiveMinibus.getPrice());
		log.info("baseCharges checked");
	}

	public void checkFuelSurcharge() {
		Assert.assertEquals(fuelSurcharge.getText(), "Fuel Surcharge");
		assertEquals(CommonFunction.getLocator("$9.00").getText(), AdditionalFees.FuelSurchargeExecutiveMinibus.getPrice());
	...
	public void checkSanitizationFee() {
		...
		assertEquals(CommonFunction.getLocator("$7.00").getText(), AdditionalFees.SanitizationFeeExecutiveMinibus.getPrice());
	...
	public void checkGratuity() {
		...
		assertEquals(CommonFunction.getLocator("$30.00").getText(), AdditionalFees.GratuityExecutiveMinibus.getPrice());
		log.info("gratuity checked");
	}

	public void checkCreditCarFee() {
		assertEquals(creditCarFee.getText(), "Credit Card Fee");
		assertEquals(CommonFunction.getLocator("$5.88").getText(), AdditionalFees.CreditCardFeeExecutiveMinibus.getPrice());
	...
	public void checkPassengerRecoveryTransportationCharge() {
		...
		assertEquals(CommonFunction.getLocator("$4.98").getText(), AdditionalFees.PassengerRecoveryTransportationChargeExecutiveMinibus.getPrice());
		log.info("Passenger Recovery Transportation Charge checked");
	}

	public void checkTotal() {
		assertEquals(total.getText(), "Total:");
		assertEquals(CommonFunction.getLocator("$206.86").getText(), "$" + Cars.ExecutiveMinibus.getPrice());
	}
