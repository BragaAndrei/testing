package OmnilimoProj.Elements.DetailsForCar.Sprinter;
...
public class HourlySprinterDetailsVehicle extends Boss {
...
//	WebElement typeOfVehicleBMW7Series;
	...
	public HourlySprinterDetailsVehicle() {
...
//	}
	...
	public void checkBaseCharges() {
		...
		assertEquals(CommonFunction.getLocator("$1,212.50").getText(), AdditionalFees.BaseChargesSprinterHourly.getPrice());
		log.info("baseCharges checked");
	}

	public void checkFuelSurcharge() {
		Assert.assertEquals(fuelSurcharge.getText(), "Fuel Surcharge");
		assertEquals(CommonFunction.getLocator("$112.50").getText(), AdditionalFees.FuelSurchargeSprinterHourly.getPrice());
	...
	public void checkSanitizationFee() {
		...
		assertEquals(CommonFunction.getLocator("$7.00").getText(), AdditionalFees.SanitizationFeeSprinterHourly.getPrice());
		log.info("Sanitization Fee checked");
	}

	public void checkAirportParking() {
		assertEquals(airportParking.getText(), "Airport Parking");
		assertEquals(CommonFunction.getLocator("$5.00").getText(), AdditionalFees.AirportParkingSprinterHourly.getPrice());
		log.info("Airport Parking checked");
	}

	public void checkGratuity() {
		assertEquals(gratuity.getText(), "Gratuity");
		assertEquals(CommonFunction.getLocator("$242.50").getText(), AdditionalFees.GratuitySprinterHourly.getPrice());
		log.info("gratuity checked");
	}

	public void checkCreditCarFee() {
		assertEquals(creditCarFee.getText(), "Credit Card Fee");
		assertEquals(CommonFunction.getLocator("$47.24").getText(), AdditionalFees.CreditCardFeeSprinterHourly.getPrice());
	...
	public void checkPassengerRecoveryTransportationCharge() {
		...
		assertEquals(CommonFunction.getLocator("$39.96").getText(), AdditionalFees.PassengerRecoveryTransportationChargeSprinterHourly.getPrice());
	...
	public void checkTotalDiffAddress() {
		...
		assertEquals(CommonFunction.getLocatorSpanTag("$1,661.70").getText(), AdditionalFees.TotalSprinterHourly.getPrice());
	}
	...
	public void checkTotal() {
		assertEquals(total.getText(), "Total:");
		assertEquals(CommonFunction.getLocator("$1.661.70").getText(), "$" + Cars.SprinterHourly.getPrice());
	}
	...
	public void checkPriceDetailsDiffAddress() throws InterruptedException {
		this.checkTypeOfVehicleSprinter();
